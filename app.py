...
def patients():
    if request.method == "POST":
        ...
        data = request.get_json(force=True)
        required = ["patient_id", "name", "age", "gender", "phone", "date_of_visit", "location_area", "main_concern"]
        ...
        return jsonify({"message": "Patient registered", "id": new_id}), 201
...
def catalog():
    if request.method == "POST":
        if session["user"]["role"] != "doctor":
            return jsonify({"error": "Only doctor can add medicine library items"}), 403
        data = request.get_json(force=True)
